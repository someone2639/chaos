...
u64 tkGetTime(void) {
    ...
    if (samples_per_sec)
        retval += samples_played * 1000000LL / samples_per_sec;
    return retval;
}
...
static void timekeeperProc(void UNUSED *argument) {
     ...
     */
    osCreateThread(&daCounterThread, DA_COUNTER_THREAD_ID, daCounterProc, NULL,
                   daCounterStack + (STACKSIZE / sizeof(u64)), (OSPri) DA_COUNTER_PRIORITY);
    ...
    osSendMesg(&tkResMesgQ, NULL,
               OS_MESG_BLOCK); /* Notification that playback preparations are finished */
    ...
    while (!video_done || videoRingCount > 0 || pushed_cfb != NULL || !audio_done || audioRingCount > 0
        ...
        if (video_started) {
            ...
            while (audioRingCount > 0 && aiFIFOsamples == 0) {
                ...
                if (++audioRingRead == AUDIO_RING_BUFFER_SIZE)
                    audioRingRead = 0;
                --audioRingCount;
            }
        ...
        if (!audio_done && audioRingCount < AUDIO_RING_BUFFER_SIZE && pcmBufferCount < NUM_PCMBUFs) {
            ...
            if (samples > 0) {
                ...
                if (++audioRingWrite == AUDIO_RING_BUFFER_SIZE)
                    audioRingWrite = 0;
                ++audioRingCount;
...
void createTimekeeper(void) {
    ...
    osCreateThread(&tkThread, TIMEKEEPER_THREAD_ID, timekeeperProc, NULL,
                   tkStack + (STACKSIZE / sizeof(u64)), (OSPri) TIMEKEEPER_PRIORITY);
