...
static u32 next_audio_record(void *pcmbuf) {
    ...
    audio_streamP = get_record(record_header, adpcmbuf, HVQM2_AUDIO, audio_streamP, &audioDmaMesgBlock,
                               &audioDmaMessageQ);
    --audio_remain;
...
void hvqm_main_proc(uintptr_t vidPtr) {
    ...
    romcpy(hvqm_header, (void *) vidPtr, sizeof(HVQM2Header), OS_MESG_PRI_NORMAL, &videoDmaMesgBlock,
           &videoDmaMessageQ);
    ...
    for (;;) {
        ...
        while (video_remain > 0) {
            ...
            if (frame_format == HVQM2_VIDEO_HOLD) {
                 ...
                 */
                hvqtask.t.flags = 0;
                status = hvqm2DecodeSP1(hvqbuf, frame_format, &gFramebuffers[bufno][screen_offset],
                                        &gFramebuffers[prev_bufno][screen_offset], hvqwork, &hvq_sparg,
                                        hvq_spfifo);
...
void createHvqmThread(uintptr_t vidPtr) {
    ...
    osCreateThread(&hvqmThread, HVQM_THREAD_ID, (void *) hvqm_main_proc, (void *) vidPtr,
