...
s32 check_galaxy_spin(struct MarioState *m) {
    if(!chaos_check_if_patch_active(CHAOS_PATCH_GALAXY_SPIN)) {
        return FALSE;
    }
    if(m->input & INPUT_A_PRESSED && !m->usedSpin && m->spinTimer > 2) {
        m->usedSpin = TRUE;
...
u32 common_air_action_step(struct MarioState *m, u32 landAction, s32 animation, u32 stepArg) {
    ...
    stepResult = perform_air_step(m, stepArg, (m->forwardVel > 16.0f) ? TRUE : FALSE);
    switch (stepResult) {
        case AIR_STEP_NONE:
...
s32 act_jump(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP_0, 0);
...
s32 act_double_jump(struct MarioState *m) {
    ...
    if (chaos_check_if_patch_active(CHAOS_PATCH_BETA)) {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP_0, SOUND_MARIO_BETA_HAA);
    } else {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP_0, SOUND_MARIO_HOOHOO);
    }
...
s32 act_spin_jump(struct MarioState *m) {
    ...
    if(common_air_action_step(m, ACT_DOUBLE_JUMP_LAND, MARIO_ANIM_TWIRL, 0) == AIR_STEP_LANDED) {
        m->angleVel[1] = 0;
    }
#if ENABLE_RUMBLE
    if (m->action == ACT_TRIPLE_JUMP_LAND) {
...
s32 act_backflip(struct MarioState *m) {
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP_0, SOUND_MARIO_YAH_WAH_HOO_0);
...
s32 act_twirling(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0, TRUE)) {
        case AIR_STEP_LANDED:
...
s32 act_water_jump(struct MarioState *m) {
    ...
    switch (perform_air_step(m, AIR_STEP_CHECK_LEDGE_GRAB, FALSE)) {
        case AIR_STEP_LANDED:
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if(m->input & INPUT_B_PRESSED && m->actionTimer > 2 && chaos_check_if_patch_active(CHAOS_PATCH_GROUND_POUND_DIVE)) {
    ...
    if (m->actionState == 0) {
        ...
        stepResult = perform_air_step(m, 0, FALSE);
...
s32 act_burning_jump(struct MarioState *m) {
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP_0, m->actionArg == 0 ? 0 : -1);
...
u32 common_air_knockback_step(struct MarioState *m, u32 landAction, u32 hardFallAction, s32 animation,
    ...
    stepResult = perform_air_step(m, 0, TRUE);
    switch (stepResult) {
        case AIR_STEP_NONE:
            set_mario_animation(m, animation);
            break;
...
s32 act_galaxy_spin(struct MarioState *m) {
    s32 animFrame;

    if (m->actionState == 0) {
        play_sound_if_no_flag(m, SOUND_MARIO_YAHOO_WAHA_YIPPEE_0 + ((gAudioRandom % 5) << 16), MARIO_ACTION_SOUND_PLAYED);
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    u32 cancel = FALSE;
    u32 resetYawVel = FALSE;
    u32 prevAction = m->action;
    ...
    if (chaos_check_if_patch_active(CHAOS_PATCH_SUNSHINE_TWIRL)) {
        ...
        if(resetYawVel && m->action != prevAction) {
            m->angleVel[1] = 0;
        }
    }
