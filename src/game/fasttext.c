...
int computeS(unsigned char letter) {
    ...
    // Lowercase j is busted for some reason
    if (idx == 64)
        return (idx << 8) + 1;
...
static const u8 fast_text_font_kerning[] = {
    /* */ 3, /*!*/ 4, /*"*/ 5, /*#*/ 5, /*$*/ 5, /*%*/ 7, /*&*/ 7, /*'*/ 2, /*(*/ 5, /*)*/ 5, /***/ 6, /*+*/ 7, /*,*/ 3, /*-*/ 7, /*.*/ 3, /*/*/ 7,
...
void drawSmallString_impl(Gfx **dl, int x, int y, const char* string, int r, int g, int b, int a) {
    int i = 0;
    int xPos = x;
    int yPos = y;
    int s = 0;
    ...
    while (string[i] != '\0') {
        ...
        if (cur_char == '\t') {
            ...
            if (cur_char != ' ') {
                s = computeS(cur_char);
                gSPTextureRectangle(dlHead++, (xPos + 0) << 2, (yPos + 0) << 2, (xPos + 8) << 2, (yPos + 12) << 2, 0, s, 0, 1 << 10, 1 << 10);
            }
