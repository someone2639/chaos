...
void reset_patch_selection_menu() {
    ...
    gPatchSelectionMenu->menu.index = 0;
...
s32 patch_select_anim_startup() {
    ...
    switch(menu->animPhase) {
        case 0:
            ...
            menu->flags |= PATCH_SELECT_FLAG_DRAW_START_TEXT;
            menu->animFrames = PATCH_SELECT_STARTUP_CURTAIN_DROP_FRAMES;

            prog = ((f32)animTimer / (f32)menu->animFrames);
            gPatchSelectionMenu->curtainPos[1] = menu_anim_f32(prog, MENU_EASE_OUT, CURTAIN_Y_START, CURTAIN_Y_POS);
        ...
        case 1:
            ...
            menu->animFrames = PATCH_SELECT_STARTUP_CARDS_SLIDE_FRAMES;
            
            prog = ((f32)animTimer / (f32)menu->animFrames);

            for(int i = 0; i < MAX_CARDS; i++) {
                struct PatchCard *card = &gPatchSelectionMenu->patchCards[i];
                f32 cur = card->pos[0];
                f32 target = card->layoutPos[0];
                card->pos[0] = approach_f32_asymptotic(cur, target, 0.27f);
            }
        ...
        case 3:
            ...
            menu->animFrames = PATCH_SELECT_STARTUP_TEXT_SLIDE_FRAMES;

            prog = ((f32)animTimer / (f32)menu->animFrames);

            gPatchSelectionMenu->selectPatchTextPos[0] = menu_anim_f32(prog, MENU_EASE_IN, SCREEN_CENTER_X, SELECT_PATCH_TEXT_END);
            break;
        default:
            //End animation, draw the cursor, hide the start text, and move to the next animation
            menu->flags &= ~PATCH_SELECT_FLAG_DRAW_START_TEXT;
            menu->flags |= PATCH_SELECT_FLAG_DRAW_CURSOR;
            menu->flags &= ~PATCH_SELECT_FLAG_HALT_INPUT;
            return TRUE;
            break;
    }
...
s32 patch_select_anim_select() {
    ...
    menu->flags |= PATCH_SELECT_FLAG_DRAW_LOWER_TEXT;
    ...
    switch(menu->animPhase) {
        case 0:
            ...
            if(gPatchSelectionMenu->numPatches == 1) {
                menu->animTimer = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_ANIM_SLIDE_FRAMES;
            }

            //Unselected cards slide off screen and halt input
            menu->flags |= PATCH_SELECT_FLAG_HALT_INPUT;
            menu->flags &= ~PATCH_SELECT_FLAG_DRAW_LOWER_TEXT;
            menu->animFrames = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_ANIM_SLIDE_FRAMES;

            prog = ((f32)animTimer / (f32)menu->animFrames);

            for(int i = 0; i < MAX_CARDS; i++) {
                if(i != gPatchSelectionMenu->selectedPatch) {
                    f32 targetX = gPatchSelectionMenu->patchCards[i].layoutPos[0];
                    if(i % 2) {
                        gPatchSelectionMenu->patchCards[i].pos[0] = menu_anim_f32(prog, MENU_EASE_IN, targetX, CARD_X_RIGHT_START);
                    } else {
                        gPatchSelectionMenu->patchCards[i].pos[0] = menu_anim_f32(prog, MENU_EASE_IN, targetX, CARD_X_LEFT_START);
                    }
                }
            }
            break;
        case 1:
            ...
            menu->animFrames = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_ANIM_SCALE_FRAMES;
            menu->flags &= ~PATCH_SELECT_FLAG_DRAW_CURSOR;

            prog = ((f32)animTimer / (f32)menu->animFrames);
            s32 selectedPatch = gPatchSelectionMenu->selectedPatch;
            f32 xStart = gPatchSelectionMenu->patchCards[selectedPatch].layoutPos[0];
            f32 yStart = gPatchSelectionMenu->patchCards[selectedPatch].layoutPos[1];

            gPatchSelectionMenu->patchCards[selectedPatch].scale = menu_anim_f32(prog, MENU_EASE_OUT, CARD_SCALE_HOVER, CARD_SCALE_SELECTED);
...
s32 patch_select_anim_confirmation_return() {
    ...
    switch(menu->animPhase) {
        case 0:
            ...
            menu->flags |= PATCH_SELECT_FLAG_HALT_INPUT;
            menu->flags &= ~PATCH_SELECT_FLAG_DRAW_LOWER_TEXT;
            menu->animFrames = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_RETURN_ANIM_SCALE_FRAMES;

            s32 selectedPatch = gPatchSelectionMenu->selectedPatch;
            f32 xEnd = gPatchSelectionMenu->patchCards[selectedPatch].layoutPos[0];
            f32 yEnd = gPatchSelectionMenu->patchCards[selectedPatch].layoutPos[1];
            prog = ((f32)animTimer / (f32)menu->animFrames);

            gPatchSelectionMenu->patchCards[selectedPatch].scale = menu_anim_f32(prog, MENU_EASE_OUT, CARD_SCALE_SELECTED, CARD_SCALE_HOVER);
        ...
        case 1:
            //If only one card, skip phase
            if(gPatchSelectionMenu->numPatches == 1) {
                menu->animTimer = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_RETURN_ANIM_SLIDE_FRAMES;
            }

            //Slides the unselected cards back onto the screen
            menu->animFrames = PATCH_SELECT_MENU_CONFIRMATION_TRANSITION_RETURN_ANIM_SLIDE_FRAMES;
            prog = ((f32)animTimer / (f32)menu->animFrames);
            ...
            for(int i = 0; i < MAX_CARDS; i++) {
                if(i != gPatchSelectionMenu->selectedPatch) {
                    f32 targetX = gPatchSelectionMenu->patchCards[i].layoutPos[0];
                    if(i % 2) {
                        gPatchSelectionMenu->patchCards[i].pos[0] = menu_anim_f32(prog, MENU_EASE_OUT, CARD_X_RIGHT_START, targetX);
                    } else {
                        gPatchSelectionMenu->patchCards[i].pos[0] = menu_anim_f32(prog, MENU_EASE_OUT, CARD_X_LEFT_START, targetX);
                    }
                }
            }
            break;
...
s32 patch_select_anim_ext_desc() {
    ...
    if(menu->animPhase == 0) {
        menu->animFrames = PATCH_SELECT_MENU_EXT_DESC_APPEAR_ANIM_FRAMES;
        menu->flags |= PATCH_SELECT_FLAG_DRAW_EXTENDED_DESCRIPTION;
        menu->flags &= ~PATCH_SELECT_FLAG_DRAW_CURSOR;
        menu->flags |= PATCH_SELECT_FLAG_HALT_INPUT;
...
s32 patch_select_menu_anim_ext_desc_return() {
    ...
    if(menu->animPhase == 0) {
        menu->animFrames = PATCH_SELECT_MENU_EXT_DESC_DISAPPEAR_ANIM_FRAMES;
        menu->flags |= PATCH_SELECT_FLAG_HALT_INPUT;
...
s32 patch_select_menu_anim_active_patches() {
    ...
    if(menu->animPhase == 0) {
        menu->animFrames = PATCH_SELECT_ACTIVE_PATCHES_CARDS_SLIDE_FRAMES;
        menu->flags &= ~PATCH_SELECT_FLAG_DRAW_CURSOR;
        menu->flags |= PATCH_SELECT_FLAG_HALT_INPUT;
        ...
        for(int i = 0; i < MAX_CARDS; i++) {
            ...
            gPatchSelectionMenu->patchCards[i].pos[0] = menu_anim_f32(prog, MENU_EASE_OUT, start, end);
        }
