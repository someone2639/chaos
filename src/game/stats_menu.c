...
ALWAYS_INLINE void stats_menu_write_playtime_stat(s32 leftAlign, s32 rightAlign, s32 yPos, const char *label, u32 time) {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, leftAlign, yPos, label, FT_FLAG_ALIGN_LEFT, 0xFF, 0xFF, 0xFF, 0xFF);
    slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, playTimeText, FT_FLAG_ALIGN_RIGHT, 0x9F, 0x9F, 0x9F, 0xFF);
}
...
ALWAYS_INLINE void stats_menu_write_basic_stat(s32 leftAlign, s32 rightAlign, s32 yPos, const char *label, u32 stat, u8 r, u8 g, u8 b) {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, leftAlign, yPos, label, FT_FLAG_ALIGN_LEFT, 0xFF, 0xFF, 0xFF, 0xFF);
    slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, statText, FT_FLAG_ALIGN_RIGHT, r, g, b, 0xFF);
}

ALWAYS_INLINE void stats_menu_write_difficulty(s32 leftAlign, s32 rightAlign, s32 yPos, u32 difficulty) {
    slowtext_draw_ortho_text(&gDisplayListHead, leftAlign, yPos, "Difficulty:", FT_FLAG_ALIGN_LEFT, 0xFF, 0xFF, 0xFF, 0xFF);
    switch(difficulty) {
        case CHAOS_DIFFICULTY_EASY:
            slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, "Easy", FT_FLAG_ALIGN_RIGHT, 0x05, 0xDf, 0x15, 0xFF);
            break;
        case CHAOS_DIFFICULTY_NORMAL:
        default:
            slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, "Normal", FT_FLAG_ALIGN_RIGHT, 0xAF, 0xAF, 0xAF, 0xFF);
            break;
        case CHAOS_DIFFICULTY_HARD:
            slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, "Hard", FT_FLAG_ALIGN_RIGHT, 0xFF, 0x15, 0x25, 0xFF);
            break;
        case CHAOS_DIFFICULTY_IMPOSSIBLE:
            slowtext_draw_ortho_text(&gDisplayListHead, rightAlign, yPos, "Impossible", FT_FLAG_ALIGN_RIGHT, 0x5F, 0x5F, 0x5F, 0xFF);
            break;
    }
}
...
ALWAYS_INLINE void stats_menu_write_favorite_patch(s32 leftAlign, s32 yPos, s32 width, const char *label, s32 patchId, u8 r, u8 g, u8 b) {
    ...
    if(patchId == CHAOS_PATCH_NONE_POSITIVE || patchId == CHAOS_PATCH_NONE_NEGATIVE) {
        slowtext_draw_ortho_text_linebreaks(&gDisplayListHead, leftAlign + 15, yPos -= STATS_PADDING, width, "---", FT_FLAG_ALIGN_LEFT, 0x7F, 0x7F, 0x7F, 0xFF);
    } else {
        sprintf(favText, "%s (%d)", favPatch->name, count);
        slowtext_draw_ortho_text_linebreaks(&gDisplayListHead, leftAlign + 15, yPos -= STATS_PADDING, width, favText, FT_FLAG_ALIGN_LEFT, r, g, b, 0xFF);
    }
}
...
void render_stats_menu_totals() {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, 0, textY -= STATS_PADDING, "TOTALS", FT_FLAG_ALIGN_CENTER, 0xFF, 0xFF, 0xFF, 0xFF);
...
void render_stats_menu_best() {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, 0, textY -= STATS_PADDING, "BEST CLEAR", FT_FLAG_ALIGN_CENTER, 0xFF, 0xFF, 0xFF, 0xFF);

    if(!gSaveBuffer.menuData.scoreData.isBestClear) {
        slowtext_draw_ortho_text(&gDisplayListHead, 0, -STATS_PADDING, "No Clear Data", FT_FLAG_ALIGN_CENTER, 0x7F, 0x7F, 0x7F, 0xFF);
    } else {
        stats_menu_write_difficulty(leftAlign, -leftAlign, textY -= STATS_PADDING, gSaveBuffer.menuData.scoreData.bestDifficulty);

        slowtext_draw_ortho_text(&gDisplayListHead, leftAlign, textY -= STATS_PADDING, "Gamemode:", FT_FLAG_ALIGN_LEFT, 0xFF, 0xFF, 0xFF, 0xFF);
        switch (gSaveBuffer.menuData.scoreData.bestGameMode) {
            case CHAOS_GAMEMODE_HARDCORE:
                slowtext_draw_ortho_text(&gDisplayListHead, -leftAlign, textY, "Hardcore", FT_FLAG_ALIGN_RIGHT, 0xAF, 0x5F, 0xCF, 0xFF);
                break;
            case CHAOS_GAMEMODE_CHALLENGE:
                slowtext_draw_ortho_text(&gDisplayListHead, -leftAlign, textY, "Challenge", FT_FLAG_ALIGN_RIGHT, 0xBB, 0xA1, 0x24, 0xFF);
                break;
            case CHAOS_GAMEMODE_CLASSIC:
            default:
                slowtext_draw_ortho_text(&gDisplayListHead, -leftAlign, textY, "Classic", FT_FLAG_ALIGN_RIGHT, 0xAF, 0xAF, 0xAF, 0xFF);
                break;
        }
...
void render_stats_menu_hardcore() {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, 0, textY -= STATS_PADDING, "HARDCORE SCORE", FT_FLAG_ALIGN_CENTER, 0xFF, 0xFF, 0xFF, 0xFF);

    if(gSaveBuffer.menuData.scoreData.bestHardcoreStars == 0) {
        slowtext_draw_ortho_text(&gDisplayListHead, 0, -STATS_PADDING, "No Hardcore Data", FT_FLAG_ALIGN_CENTER, 0x7F, 0x7F, 0x7F, 0xFF);
    } else {
...
void render_stats_conf_delete() {
    ...
    slowtext_draw_ortho_text(&gDisplayListHead, 0, 0, "Are you sure you want to delete all saved statistics?", FT_FLAG_ALIGN_CENTER, 0xFF, 0xFF, 0xFF, 0xFF);
...
void render_stats_button_prompts() {
    ...
    menu_render_button_prompt_list(&gDisplayListHead, SCREEN_WIDTH - 35, SCREEN_HEIGHT - 33, &prompts);
}
